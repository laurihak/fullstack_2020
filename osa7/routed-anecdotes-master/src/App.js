...
const AnecdoteList = ({ anecdotes }) => (
  <div>
    <h2>Anecdotes</h2>
    <ul>
      {anecdotes.map(anecdote => <Link to={`/anecdotes/${anecdote.id}`} key={anecdote.id} >{anecdote.content}<br></br></Link>)}
    </ul>
  </div>
)
  ...
  if (anecdote)
    return (
      <div>
        ...
        <div>Written by {anecdote.author}</div>
        <div>has {anecdote.votes} votes</div>
        <a href={anecdote.info} alt='link not found'>{anecdote.info}</a>
      </div>
    )
  return (
    <div>
